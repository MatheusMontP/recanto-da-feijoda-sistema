            ...
            if data.get("code") == "Ok" and "distances" in data and "durations" in data:
                ...
                for i in range(n):
                    ...
                    for j in range(n):
                        ...
                        # Coordenadas reais para o fallback
                        ...
                        # SANITY CHECK: Se o OSRM der uma volta maior que 2.5x a linha reta, algo está errado
                        if osrm_dist_km == float('inf') or osrm_dist_km > (h_dist * 2.5):
                            fallback_dist = h_dist * 1.3
                            osrm_dist_km = fallback_dist
                            val_dur = (fallback_dist / 25.0) * 3600.0

                        # BÔNUS DE BAIRRO (Simples e Direto)
                        if i > 0 and j > 0 and i != j:
                            addr_i = nodes[i-1]["address"].lower()
                            addr_j = nodes[j-1]["address"].lower()
                            bairros = ["santa maria", "atalaia", "aruana", "aeroporto", "farolandia", "jardins", "grageru"]
                            for b in bairros:
                                if b in addr_i and b in addr_j:
                                    osrm_dist_km *= 0.1  # Força ficarem juntos
                                    val_dur *= 0.1
                                    break
                ...
                raise ValueError("Resposta OSRM inválida.")
    except Exception as exc:
        logger.error("Erro OSRM: %s", exc)
        raise HTTPException(
            status_code=503,
            detail="Serviço de cálculo de rotas (OSRM) indisponível."
        )
